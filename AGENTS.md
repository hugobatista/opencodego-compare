...
Display metadata (badge/plan labels, provider, URLs, subscription price, badge colors) is **not hardcoded in the frontend** — it lives in `fetch/plans.json`, which `build_json.py` copies into `prices.json` `meta.providers`/`meta.plans`. The frontend iterates `meta.plans` for badges/legend/provider/plan colors; rows carry `plan` (label) and the market key must match a `meta.plans` key. Adding a new provider = a `plans.json` entry + a scraper + a data file; no frontend changes.

## Config

- `fetch/plans.json`: providers (name/url) and plans (name, provider, url, `subPrice`, `feeTax`, `color`/`colorDark`). Single source for all frontend display metadata.
