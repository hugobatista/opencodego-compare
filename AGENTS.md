...
- `fetch_modelmarkets.py` fetches the models sitemap (allowed by robots.txt), caches `data/modelmarkets.json`, and re-fetches only pages whose sitemap `lastmod` changed. It regex-extracts the Hugging Face repo from each page (`hf` field) — only open-weight models have one. In `build_json.py` modelmarkets is now a **fallback-only** source (L3 below), not the maker authority.
- Identity resolution: `build_json.py` `add_model_links`/`identity_for_row` resolves `maker`/`makerLink`/`developerId`/`hfLink`/`modelMarketsLink`/`variantLink` in layered order for every row. **L4 `fetch/catalog_overrides.json`** (manual, highest authority; keys are normalized name or raw `base` alias — see `ling3.0flashfin`, `gemma4e4bit`) provides any field when automation is wrong or missing. **L1 OpenRouter** (`openrouter.json` `id` org + `name` minus `Maker: ` prefix + `hugging_face_id`, matched exact then containment for 8+ chars) supplies maker/makerLink/developerId/variantLink/hf. **L2 DeepInfra** (org parsed from `variantLink`, e.g. `deepinfra.com/XiaomiMiMo/...`) supplies maker for DI fine-tunes. **L3 modelmarkets** supplies `hfLink`/`modelMarketsLink` only — never maker. Unresolved rows print an `Identity warning` to stderr and are written to `data/unmapped.json` (deleted when empty); grow `catalog_overrides.json` to clear them. OpenRouter rows keep their builder's maker/developerId/variantLink and only gain `hfLink`/`modelMarketsLink` (plus override `hfLink` when the OR API lacks `hugging_face_id`). Rows get `maker`/`makerLink` from `maker_urls.json`/`makers.json` by org; add new org aliases there (e.g. `XiaomiMiMo`, `stepfun-ai`).
